...
def slots():
    ...
    for i,s in enumerate(result):
        ...
        s['offspring_total_max']=s['batch']*2 if s['mode'] in ['descendant','fanout'] else (1 if s['mode']=='wait' else 0)
    return result
...
def freeze(output,native_build,n4_build):
    ...
    proposal={'revision':'N4_OWNER_CHANGE_PLAN_V1','approved':False,'status':'PROPOSAL_NOT_EXECUTED','head':head,'source':source,'host':cap['identity'],'binaries':binaries,'inputs':inputs,
              ...
              'scope':'Agent cgroup writes only to dedicated service descendants; no manual shared root/user@ edits, installs, cloud/API/IAM or global cache changes. PID1 may enable required ancestor controllers as the normal hierarchical effect of the exact systemd service request.',
              'read_only_observations':['/proc/vmstat','/proc/pressure/memory','/proc/meminfo','systemctl show the exact service (Result,ExecMainCode,ExecMainStatus,ActiveState,SubState,ControlGroup)','unprivileged journalctl -k -b -n 50 --no-pager --output=json --grep=oom-kill|Killed process|Out of memory; missing permission stays UNKNOWN'],

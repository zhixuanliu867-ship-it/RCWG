...
static void record_process(){
    std::ifstream file("/proc/self/cgroup");std::string group;std::getline(file,group);
    cpu_set_t set;CPU_ZERO(&set);if(sched_getaffinity(0,sizeof(set),&set))std::exit(76);
    std::string affinity;for(int i=0;i<CPU_SETSIZE;i++)if(CPU_ISSET(i,&set))affinity+=std::to_string(i)+",";
    std::string raw=std::to_string(getpid())+"\n"+std::to_string(getppid())+"\n"+group+"\n"+affinity+"\n";
    std::string name="process-"+std::to_string(getpid())+".txt";
    int fd=open(name.c_str(),O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW,0600);
    if(fd<0||write(fd,raw.data(),raw.size())!=(ssize_t)raw.size()||fsync(fd)||close(fd))std::exit(77);
}
...
int main(int argc,char**argv){
    ...
    for(int b=0;b<batch;b++){
        if(mode=="noop")cpu(1);
            ...
            pid_t children[2];for(int i=0;i<2;i++){children[i]=fork();if(children[i]<0)return 67;if(children[i]==0){record_process();memory(8*1024*1024,50);cpu(30000000);_exit(0);}}
        ...
        else if(mode=="wait"){pid_t p=fork();if(p<0)return 70;if(!p)record_process();usleep(5000000);if(!p)_exit(0);int st;waitpid(p,&st,0);}
        else return 71;
    }

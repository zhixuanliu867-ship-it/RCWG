...
from .ir_structure import analyze_structure, parse_ir_bytes, _json_value
...
def _compile(task, plan, public, structure, stage):
    ...
    def visit(nodes, scope, pointer, bound, yield_map=None):
        ...
        for name in scope_orders[scope]:
            ...
            checked = validate_operator(node, {k: v.type for k, v in inputs.items()},
            ...
            typed_nodes.append({'id': qid, 'scope': scope, 'logical_id': name,
                                ...
                                'params': deepcopy(node['params']), 'after': list(node.get('after', [])),

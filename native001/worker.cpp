...
struct Tracking {
    bool enabled=false;int64_t created=0,released=0,live=0,peak=0,last=now();double byte_seconds=0;
    void change(int64_t delta,bool allocation){if constexpr(instrumentation){if(enabled){auto t=now();byte_seconds+=double(live)*double(t-last)/1e9;last=t;live+=delta;peak=std::max(peak,live);if(allocation)++created;else ++released;}}}
};
struct Buffer {
    uint64_t id;std::vector<J> cells;std::shared_ptr<Tracking> tracking;int64_t tracked_bytes=0;
    void publish(const std::shared_ptr<Tracking>& t){
        if constexpr(instrumentation){if(t->enabled){
            tracking=t;tracked_bytes=int64_t(sizeof(Buffer)+cells.capacity()*sizeof(J));
            for(auto& cell:cells)if(cell.string()){
                auto p=reinterpret_cast<uintptr_t>(cell.str().data()),lo=reinterpret_cast<uintptr_t>(&cell);
                if(p<lo||p>=lo+sizeof(J))tracked_bytes+=int64_t(cell.str().capacity()+1);
            }
            tracking->change(tracked_bytes,true);
        }}
    }
    ~Buffer(){if(tracking)tracking->change(-tracked_bytes,false);}
};
struct Row {
...
struct Ctx {
    ...
    void witness(const std::string& node,uint64_t from,uint64_t to,size_t cells){
        if constexpr(instrumentation){if(diagnostic&&std::count_if(ownership.begin(),ownership.end(),[&](const J& v){return v.at("node_id").str()==node;})<8)ownership.push_back(J::O{{"node_id",node},{"source_buffer_id",int64_t(from)},{"result_buffer_id",int64_t(to)},{"selected_cells",int64_t(cells)}});}
    }
};
...
static Row project(const Row& input,const J& columns,bool copy,Ctx& ctx,const std::string& id){
    ...
    if(copy){
        ...
        b->publish(ctx.tracking);r.backing=b;ctx.add(id,"row_buffers_copied");ctx.add(id,"new_buffer_allocations");
    }else{
...
class Scan:public Stream {
    ...
    bool pull(Row& row)override{
        ...
        while(file.line(line)){
            ...
            b->publish(ctx.tracking);input.backing=b;ctx.add(id,"rows_scanned");ctx.add(id,"row_buffers_allocated");
...
int main(int argc,char** argv){
    ...
    try{
        ...
        ctx.tracking->enabled=ctx.diagnostic;
    ...
    report["buffer_lifecycle"]=ctx.diagnostic?J(J::O{{"registered_created",ctx.tracking->created},{"registered_released",ctx.tracking->released},{"registered_live_bytes_final",ctx.tracking->live},{"registered_peak_capacity_bytes",ctx.tracking->peak},{"registered_byte_seconds",ctx.tracking->byte_seconds},{"scope","Published immutable row buffers and explicit string/vector capacity; excludes parser temporaries, allocator overhead and library copies; not whole-worker RAM"}}):J(nullptr);

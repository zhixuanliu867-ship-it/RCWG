...
def literal_type(value, path):
    ...
    if type(value) is list:
        if not value:
            return Type("List", item=Type("Empty"), metadata={"max_length":0})
        ...
        return Type("List", item=_lift(first, len(nonnull) != len(items)), metadata={"max_length":len(value)})
...
def analyze_predicate(ast, schema: dict[str, Type], path: str, *, require_bool=True):
    ...
    def temporal_literal(node, inferred, expected, p):
        ...
        return Type("List", item=item, metadata={"max_length":len(raw)}) if type(raw) is list else item
